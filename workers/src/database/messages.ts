...
export async function insertMessage(
  ...
  await db
    .prepare("UPDATE stats SET value = value + 1 WHERE key = 'messages'")
    .run();
}
...
export async function getMessagesSinceId(
  ...
  const sinceMsg = await db
    .prepare(`SELECT time FROM messages WHERE mid = ?`)
    .bind(sinceId)
    .first<{ time: number }>();
...
export async function deleteExpiredMessages(db: D1Database): Promise<number> {
  ...
  const result = await db
    .prepare(`DELETE FROM messages WHERE expires <= ?`)
    .bind(now)
    .run();
