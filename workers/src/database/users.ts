import type { User, UserRow, Token, TokenRow, UserAccess, UserAccessRow } from '../types/user';
...
function generateTokenId(): string {
  return 'tk_' + generateMessageId() + generateMessageId() + generateMessageId();
}
...
// Create a new user
export async function createUser(db: D1Database, username: string, password: string, role: User['role'] = 'user'): Promise<User> {
...
export async function getUserPasswordHash(db: D1Database, id: string): Promise<string | null> {
  const row = await db.prepare('SELECT password_hash FROM users WHERE id = ?').bind(id).first<{ password_hash: string }>();

  return row?.password_hash ?? null;
}

// Update user password
export async function updateUserPassword(db: D1Database, id: string, newPassword: string): Promise<void> {
...
export async function usernameExists(db: D1Database, username: string): Promise<boolean> {
  const row = await db.prepare('SELECT 1 FROM users WHERE username = ? COLLATE NOCASE').bind(username).first();

  return row !== null;
}

// Create a new token for a user
export async function createToken(db: D1Database, userId: string, label: string = '', expiresIn: number = 0): Promise<Token> {
  const id = generateTokenId();
  const now = Math.floor(Date.now() / 1000);
  const expires = expiresIn > 0 ? now + expiresIn : 0;

  await db.prepare(`INSERT INTO tokens (id, user_id, label, last_access, last_origin, expires) VALUES (?, ?, ?, ?, '', ?)`).bind(id, userId, label, now, expires).run();
...
export async function getUserTokens(db: D1Database, userId: string): Promise<Token[]> {
  const result = await db.prepare('SELECT id, user_id, label, last_access, last_origin, expires FROM tokens WHERE user_id = ?').bind(userId).all<TokenRow>();
...
// Delete a token
export async function deleteToken(db: D1Database, tokenId: string, userId: string): Promise<boolean> {
  const result = await db.prepare('DELETE FROM tokens WHERE id = ? AND user_id = ?').bind(tokenId, userId).run();
...
export async function setUserAccess(db: D1Database, userId: string, topic: string, read: boolean, write: boolean): Promise<void> {
  await db
    .prepare(
    ...
    .bind(userId, topic, read ? 1 : 0, write ? 1 : 0, read ? 1 : 0, write ? 1 : 0)
    .run();
}

// Delete user access for a topic
export async function deleteUserAccess(db: D1Database, userId: string, topic: string): Promise<void> {
  await db.prepare('DELETE FROM user_access WHERE user_id = ? AND topic = ?').bind(userId, topic).run();
}

// Check if a topic is reserved
export async function isTopicReserved(db: D1Database, topic: string): Promise<boolean> {
  const row = await db.prepare('SELECT 1 FROM reservations WHERE topic = ?').bind(topic).first();

  return row !== null;
}

// Get topic reservation owner
export async function getTopicOwner(db: D1Database, topic: string): Promise<string | null> {
  const row = await db.prepare('SELECT user_id FROM reservations WHERE topic = ?').bind(topic).first<{ user_id: string }>();
